...
export default function EditDJServicePage() {
    ...
    useEffect(() => {
        fetch(`/api/admin/dj-services/${params.id}`)
            .then(r => r.json())
            .then(data => {
                if (data.id) {
                    setName(data.name || "");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/admin/dj-services/${params.id}`, {
                method: "PATCH",
    ...
    if (fetching) return <div className="p-20 text-center animate-pulse text-slate-400">Loading details...</div>;

    return (
        <div className="max-w-4xl px-4 py-8 mx-auto">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-black tracking-tighter">Edit DJ Service</h1>
                    <p className="text-sm text-slate-500">Update pricing and details.</p>
                </div>
                <Link href="/admin/dj-services" className="px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-slate-900 transition-all border border-slate-200 bg-white shadow-sm">Cancel Changes</Link>
            </div>
                ...
                <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        type="number" value={basePrice || 0} onChange={e => setBasePrice(Number(e.target.value))} required min="0" step="0.01"
                    ...
                    <div className="space-y-6 bg-[#123A2F] p-8 rounded-3xl md:col-span-2">
                        ...
                        <div className="flex justify-end pt-4 mt-4 border-t border-white/10">
                            ...
                            >
                                {loading ? "Saving Changes..." : "Update DJ Service"}
                            </button>
                        </div>
                    </div>
                </div>
